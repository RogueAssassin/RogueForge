...
class RogueForgeTests(unittest.TestCase):
 ...
 def test_v140_logging_operations_and_github_alignment(self):
  ...
  self.assertIn('RELEASE-1.9.0%20TESTING',readme);self.assertIn('## Rogue ecosystem',readme)
  self.assertIn('## 1.9.0 — Pre-2.0 production cleanup',road);self.assertIn('## 1.4.0 (testing)',change)
 ...
 def test_pre20_rogue_dashboard_integration(self):
  ...
  self.assertNotIn('append_env_revision 160',update);self.assertIn('pre-2.0 production cleanup',readme);self.assertIn('## 1.9.0 — Pre-2.0 production cleanup',road)
 def test_v190_pre20_cleanup(self):
  ...
  self.assertIn('## 1.9.0 — Pre-2.0 production cleanup',road);self.assertIn('## 2.0.0 — Stable operations platform',road)
  self.assertIn('## 1.9.0 (testing)',change);self.assertIn('final 1.x cleanup baseline',change)
  self.assertIn('RELEASE-1.9.0%20TESTING',readme);self.assertIn('pre-2.0 production cleanup',readme)
 def test_current_release_baseline(self):
  self.assertEqual((ROOT/'VERSION').read_text().strip(),'1.9.0')
  ...
  self.assertIn('## 1.9.0 — Pre-2.0 production cleanup',road)
