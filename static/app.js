...
const rfOldRenderAll=renderAll;renderAll=function renderAllV080(){rfOldRenderAll();rfBranding();renderOverview();renderStacks();renderContainers();const v=$('#rfSidebarVersion');if(v&&state.status?.appVersion)v.textContent=`v${state.status.appVersion}`;const fv=$('#rfFooterVersion');if(fv&&state.status?.appVersion)fv.textContent=`v${state.status.appVersion}`;};
