...
(()=>{
  ...
  function ensureDrawer(){
    ...
    document.getElementById('rfOperationsList').addEventListener('click',async e=>{const b=e.target.closest('[data-rf-cancel-op]');if(!b)return;b.disabled=true;try{await nativeFetch(`/api/operations/${encodeURIComponent(b.dataset.rfCancelOp)}`,{method:'DELETE',headers:window.authState?.csrf?{'x-csrf-token':window.authState.csrf}:{},credentials:'same-origin'});await syncOperations();}finally{b.disabled=false;}});
  }
